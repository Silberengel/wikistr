...
/**
 * Convert AsciiDoc content to FODT (Open Document Format)
 */
export async function exportToFODT(options: ExportOptions): Promise<Blob> {
  const baseUrl = ASCIIDOCTOR_SERVER_URL.endsWith('/') ? ASCIIDOCTOR_SERVER_URL : `${ASCIIDOCTOR_SERVER_URL}/`;
  const url = `${baseUrl}convert/fodt`;
  const response = await fetch(url, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
  ...
  if (!response.ok) {
    ...
    if (contentType.includes('application/json')) {
      const error = await response.json().catch(() => ({ error: 'Unknown error' }));
      throw new Error(error.error || error.message || `Failed to generate FODT: ${response.statusText}`);
    }
    throw new Error(`Failed to generate FODT: ${response.status} ${response.statusText}`);
  }

  // Verify we got a FODT response
  const contentType = response.headers.get('content-type') || '';
  if (!contentType.includes('application/vnd.oasis.opendocument.text') && !contentType.includes('application/xml') && !contentType.includes('text/xml')) {
    // Might be an error response, try to read as JSON or text
    const text = await response.text();
    // Check if it's the SvelteKit app shell (common error when proxy fails)
    if (text.includes('__sveltekit') || text.includes('sveltekit-preload-data')) {
      throw new Error('Server returned SvelteKit app shell instead of FODT. Check AsciiDoctor server is running at /asciidoctor/ and proxy is configured correctly.');
    }
    try {
      const error = JSON.parse(text);
      throw new Error(error.error || error.message || 'Server returned non-FODT response');
    } catch {
      throw new Error(`Server returned unexpected content type: ${contentType}. Response preview: ${text.substring(0, 200)}`);
    }
  }
