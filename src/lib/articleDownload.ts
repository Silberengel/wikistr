...
async function buildAsciiDocWithMetadata(
  ...
  // For PDF, abstract should be on its own page with the header on that page
  // Use page break (<<<) to ensure abstract is on separate page, and don't use [abstract] attribute
  // as it moves the header to the title page
...
export async function combineBookEvents(
  ...
  if (isTopLevel && indexEvent.kind === 30040) {
    ...
    if (metadataFields.length > 0) {
      // Add page break before book metadata to prevent it from bleeding into title page
      // The title page is auto-generated by :doctype: book, so we need to separate the metadata
      doc += '\n<<<\n\n';
      doc += `[.book-metadata]\n== ${displayTitle}\n\n`;
