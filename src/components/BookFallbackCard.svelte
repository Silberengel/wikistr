<script lang="ts">
  ...
  interface Props {
    ...
    // Function to generate URL for a single reference
    generateUrlForReference: (ref: BookReference, version?: string) => string | null;
...
{#if parsedQuery && parsedQuery.references.length > 0}
    ...
    <div class="space-y-4">
      {#each parsedQuery.references as ref}
        ...
        <div class="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
          {#if refLoading || (!hasAttemptedLoad && refServiceUrl && !refPreview && !refError)}
            ...
            <div class="flex items-center gap-2 text-xs text-gray-500">
              ...
              <span>Loading preview from {serviceName}...</span>
            </div>
          {:else if refPreview}
            <div class="space-y-3">
              <div class="space-y-1">
              ...
              <div class="text-sm text-gray-700 leading-relaxed">
                {refPreview.description || `Preview generated by ${serviceName}`}
              </div>
            </div>
          {:else if refError}
            <div class="text-xs text-gray-500">OpenGraph data could not be fetched. Use the button below to view on {serviceName}.</div>
          {:else if !refServiceUrl}
            <div class="text-xs text-amber-600">Could not generate {serviceName} URL for {formatReference(ref)}</div>
          {:else}
      ...
      {#if serviceUrl}
        <div class="flex justify-center pt-2">
          ...
          >
            ...
            View All Passages on {serviceName}
          </a>
        </div>
      {/if}
    </div>
  ...
  {/if}
{:else}
  <!-- Generic fallback for non-book references -->
  {#if noWrapper}
    <div class="text-xs text-gray-400 italic leading-relaxed" style="margin-bottom: 1rem;">
      The search result was not found on the relays.
    </div>
    <div class="space-y-2">
      {#each parsedQuery?.references || [] as ref}
        <div class="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
          ...
          <div class="text-xs text-gray-500">
            No results found for this reference on the relays.
          </div>
        </div>
      {/each}
    </div>
