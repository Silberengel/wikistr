...
async function loadBibleGatewayPreview() {
  const targetUrl = generateBibleGatewayUrlFromQuery(query);
  if (!targetUrl) {
    ogPreview = null;
    ogError = null;
    ogLoadedQuery = query;
  ...
  try {
    ogPreview = await fetchBibleGatewayOgFromUrl(targetUrl);
    ogLoadedQuery = query;
  } catch (error) {
...
<div class="book-search-results">
  ...
  {#if loading}
    <div class="flex items-center gap-2 text-xs text-gray-500 mt-4">
      <div class="book-search-spinner"></div>
      Gathering more {bookTypeDisplayName.toLowerCase()} passages...
    </div>
  {/if}
  {#if versionNotFound}
    ...
    {#if versionNotFound}
      <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
        ...
        <div class="mt-2 text-sm text-yellow-700">
          The requested version "{parsedQuery?.version}" was not found. The BibleGateway preview replaces the old fallback cards.
        </div>
      </div>
    {:else}
